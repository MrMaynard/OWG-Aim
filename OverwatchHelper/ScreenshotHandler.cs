...
namespace OverwatchHelper
{
    ...
    public class CaptureScreen
    {
        ...
        public static Bitmap GetDesktopImage(int scale)
        {
            //In size variable we shall keep the size of the screen.
            SIZE size;
            ...
            //the screen device context.
            hBitmap = PlatformInvokeGDI32.CreateCompatibleBitmap
                        (hDC, size.cx / scale, size.cy / scale);
            ...
            if (hBitmap != IntPtr.Zero)
            {
                ...
                IntPtr hOld = (IntPtr)PlatformInvokeGDI32.SelectObject
                                        (hMemDC, hBitmap);
                //We copy the Bitmap to the memory device context.
                PlatformInvokeGDI32.BitBlt(hMemDC, 0, 0, size.cx / scale, size.cy / scale, hDC,
                                            (size.cx / 2) - size.cx / (scale * 2), (size.cy / 2) - size.cy / (scale * 2), PlatformInvokeGDI32.SRCCOPY);
                //We select the old bitmap back to the memory device context.
                PlatformInvokeGDI32.SelectObject(hMemDC, hOld);
                //We delete the memory device context.
                PlatformInvokeGDI32.DeleteDC(hMemDC);
        ...
        public static Moment GetDesktopMoment(int scale)
        {
            //In size variable we shall keep the size of the screen.
            SIZE size;
